...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussxuanchuanshipinCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussxuanchuanshipinCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussxuanchuanshipin', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussxuanchuanshipin');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discussxuanchuanshipin');
			...
			async init(){
				...
				let res = await this.$api.info('discussxuanchuanshipin', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussxuanchuanshipin', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussxuanchuanshipin/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
