	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussxuanchuanshipin`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussxuanchuanshipin`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussxuanchuanshipin`, this.ruleForm);
						}else{
							await this.$api.add(`discussxuanchuanshipin`, this.ruleForm);
						}
						uni.setStorageSync('discussxuanchuanshipinCleanType',true);
