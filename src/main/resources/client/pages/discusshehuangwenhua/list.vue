			...
			<view class="uni-product-list" :style='{"padding":"24rpx 24rpx 24rpx 24rpx","margin":"0px 0 0 0","alignItems":"flex-start","flexWrap":"wrap","display":"flex","width":"100%","justifyContent":"space-between","height":"auto","order":"3"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"margin":"0 0 40rpx 0","borderRadius":"8rpx","flexWrap":"wrap","textAlign":"center","background":"none","display":"flex","width":"48%","justifyContent":"center","height":"auto"}' v-for="(product,index) in list" :key="index">
					<view :style='{"padding":"0","margin":"8rpx 0 0 0","display":"flex","width":"100%","justifyContent":"space-between","height":"auto","order":"3"}'>
						<view :style='{"border":"0px solid #dcf8f5","padding":"0px 20rpx 20rpx 20rpx","borderRadius":"60rpx","display":"flex"}' v-if="(userid && isAuth('discusshehuangwenhua','修改')) || (!userid && isAuthFront('discusshehuangwenhua','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"border":"0px solid #f9dede","padding":"0px 20rpx 20rpx 20rpx","borderRadius":"60rpx","display":"flex"}' v-if="(userid && isAuth('discusshehuangwenhua','删除')) || (!userid && isAuthFront('discusshehuangwenhua','删除'))" @click.stop="onDeleteTap(product.id)">
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`discusshehuangwenhua`, params);
                } else {
                    res = await this.$api.list(`discusshehuangwenhua`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('discusshehuangwenhua', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`discusshehuangwenhua`, searchForm);
                } else {
                    res = await this.$api.list(`discusshehuangwenhua`, searchForm);
                }
				// 如果是第一页数据置空
