	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussyinletuijian`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussyinletuijian`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussyinletuijian`, this.ruleForm);
						}else{
							await this.$api.add(`discussyinletuijian`, this.ruleForm);
						}
						uni.setStorageSync('discussyinletuijianCleanType',true);
