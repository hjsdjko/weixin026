...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussyinletuijianCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussyinletuijianCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussyinletuijian', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussyinletuijian');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discussyinletuijian');
			...
			async init(){
				...
				let res = await this.$api.info('discussyinletuijian', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussyinletuijian', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussyinletuijian/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
