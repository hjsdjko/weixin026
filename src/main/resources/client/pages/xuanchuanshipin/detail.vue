	...
	<view class="content">
		<view class="container" :style='{"width":"100%","padding":"0px","position":"relative","background":"#ff993310","height":"auto"}'>
				...
				<view class="detail-list-item video" :style='{"margin":"0 0 24rpx 0","borderColor":"#cccccc","borderWidth":"0 0 0 0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}'>
					<video :style='{"border":"0","width":"100%","margin":"0 0 0 0","outline":"none","borderRadius":"8rpx","display":"block"}' id="myVideo" :src="baseUrl+detail.xuanchuanshipin" controls></video>
				</view>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			this.getThumbsup();
			let cleanType = uni.getStorageSync('discussxuanchuanshipinCleanType')
			if(cleanType){
				uni.removeStorageSync('discussxuanchuanshipinCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('xuanchuanshipin', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','xuanchuanshipin');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','xuanchuanshipin');
			...
			async init(){
				...
				let res = await this.$api.info('xuanchuanshipin', this.id);
				this.detail = res.data;
				// 轮播图片
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussxuanchuanshipin', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussxuanchuanshipin/add-or-update?refid=${this.id}`)
			},
			// 获取赞踩
			async getThumbsup() {
				if(!this.user){
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								...
								await _this.$api.update('xuanchuanshipin', _this.detail);
