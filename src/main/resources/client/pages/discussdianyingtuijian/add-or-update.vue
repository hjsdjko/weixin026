	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussdianyingtuijian`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussdianyingtuijian`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussdianyingtuijian`, this.ruleForm);
						}else{
							await this.$api.add(`discussdianyingtuijian`, this.ruleForm);
						}
						uni.setStorageSync('discussdianyingtuijianCleanType',true);
