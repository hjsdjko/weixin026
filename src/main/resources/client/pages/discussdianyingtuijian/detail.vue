...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussdianyingtuijianCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussdianyingtuijianCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussdianyingtuijian', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussdianyingtuijian');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discussdianyingtuijian');
			...
			async init(){
				...
				let res = await this.$api.info('discussdianyingtuijian', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussdianyingtuijian', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussdianyingtuijian/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
